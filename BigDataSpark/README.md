...
Это моё решение второй лабораторной работы по Анализу Больших Данных на применение Apache Spark. 

Для того, чтобы запустить лабораторную работу, необходимо запустить Docker контейнеры через

```bash
docker compose up -d
```

и, чтобы посмотреть сам процесс преобразования данных и примеры, откройте в браузере следующую ссылку: http://localhost:8888/lab/tree/notebooks/etl_pipeline.ipynb
