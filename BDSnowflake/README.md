...
Это моё решение второй лабораторной работы по Анализу Больших Данных на нормализацию данных в снежинку.

Для того, чтобы запустить лабораторную работу, достаточно запустить Docker контейнеры через Compose:

```bash
docker compose up
```

Скрипт для валидации расположен в `sql/05_validation.sql`, он запускается автоматически при запуске контейнеров.

Если нужно вставить дополнительный скрипты, вставляйте их в папку `sql` с приписанным слева порядковым номером (оно должно быть больше пяти, чтобы основные скрипты правильно отработали).
